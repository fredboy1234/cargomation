...
if(!empty($emails)){ 
    ...
    foreach($emails as $key=>$email){
		...
		if(strpos(htmlentities($overview->subject),'HTML to Hub') !== false){
        ...
        $message = imap_qprint(imap_fetchbody($imapResource, $email, 1.1));	
		
		$job = 		trim(strval(get_data($message, "&ltship&gt", "&lt/ship&gt")));		
		$shipper =  trim(strval(get_data($message, "&ltshipper&gt", "&lt/shipper&gt")));		
		$client =   trim(strval(get_data($message, "&ltclient&gt", "&lt/client&gt")));	
		$mbill = 	trim(strval(get_data($message, "&ltmbill&gt", "&lt/mbill&gt")));		
		$hbill = 	trim(strval(get_data($message, "&lthbill&gt", "&lt/hbill&gt")));	
	    $mcode = 	trim(strval(get_data($message, "&ltmacro&gt", "&lt/macro&gt")));
