...
class Shipment extends Core\Controller {
    ...
    public function searchFilter($user_id, $role_id) {
        // $user = $_POST['data'][0]['value'];
        // $role = $_POST['data'][1]['value'];
        $User = Model\User::getInstance($user_id);
        $user_key = $user_id;
        $shipment_id = $this->Shipment->getShipment($user_id, "shipment_num");
        ...
        switch ($role_id) {
            ...
            case 2: // CLIENT ADMIN
                break;
            case 3: // STAFF
                $user_key = $User->getSubAccountInfo($user_id)[0]->account_id;
                break;
            case 4: // CUSTOMER
                $org_code = Model\User::getUserInfoByID($user_id)[0]->organization_code;
                if($org_code == NULL) {
                    $org_code = $User->getUserContactInfo($user_id)[0]->organization_code;
                }
                $shipment_id = $this->Shipment->getShipmentByOrgCode($org_code, "shipment_num");
                $user_key = $User->getSubAccountInfo($user_id)[0]->account_id;
                $data = $org_code;
        ...
        $shipment_link = $this->Shipment->getShipmentLink($user_id);
        $doc_type = array_column($this->Document->getDocumentType(), 'type');
        $data = $docsCollection = $json_data = $html = $tableData = $searchStore = array();
    
        # Advance search
        // if(isset($_POST['post_trigger']) && $_POST['post_trigger'] != ""){
            $searchResult = $this->Shipment->searchFilter($_POST);
