...
$(document).ready(function () {
  ...
  $('#addvance-search-form').on("submit", function(ev) {
    ...
    if($("select[name='cond[]']").length > 1 && $('#no_cond_1').val() == '') {
      allBlank = true;
    }
    if(allBlank) {
      ...
      Swal.fire(
        'Search failed!',
        `Please fill in all the query field!`,
        'warning'
      )
    } else {
  ...
  $(document).on('click', '.doc', function (e) {
    ...
    var url = "/shipment/document/" + $(this).data('id') + type;
    preloader(url);
    ...
    if (wasClicked) {
      if (map.length > 0) {
        $.each(map, function (okey, oval) {
          var column = $('.table').DataTable().column(oval);
          column.visible(column);
        });
      } else {
        var column = $('.table').DataTable().columns();
        column.visible(!column.visible());
      }
      wasClicked = false;
    } else {
      var column = $('.table').DataTable().column(last);
      column.visible(!column.visible());
    }
    byLevel();
  }, 100);
...
byLevel();
$.each(getSettings(), function ($key, $val) {
  if ($val.index_check == false) {
    var column = $('.table').DataTable().column($val.index_value);
    column.visible(!column.visible());
  }
});
...
function initColumnFilter() {
  ...
  var customSettings = $('#settings').bootstrapDualListbox('getContainer');
...
function clearFilter() {
  ...
  var input = '<input name="value[]" id="no_value_1" type="text" class="form-control w_90" placeholder="Enter search value">';
  $(".form_field_outer_row:not(:first)").remove();
  $("#no_value_1").val("");
  $("#no_search_1").val("");
  $(".form_field_outer_row div.form-group:nth-child(3)").html(input);
    ...
    if (localStorage.getItem("defaultSearch")) {
      ...
      $(defaultSetting).each(function(k,v){
        ...
        setTimeout(function(){
          ...
          $("#container_mode_"+xdex).change();
          setTimeout(function(){
            $("#container_mode_"+xdex).val(v.text).trigger('change');
          },400);
        },400);
      });
    }
