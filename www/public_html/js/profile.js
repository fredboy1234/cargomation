$(document).ready(function () {
    ...
    $('#colorSchemev1').spectrum({
        type: "component",
        allowEmpty:true
    });

    $('#setcolor').on('click',function(){
        var color = $('#colorSchemev1').val();
        var styleJson = '';
        var stylevaribale = [];
        console.log(color);
        if(color ==="" ){
            ...
            for(var i=0;i<=4;i++){
                var colorer = ColorLuminance(color, i/3);
                $("#pal-"+i).css("background-color",colorer);
                stylevaribale.push(colorer);
             }
        }
        $.ajax({
            type: "POST",
            url: document.location.origin + "/profile/setColorScheme",
            data: {'colorset':stylevaribale},
            dataType: "JSON",
            success: function (res) {
                console.log(res);
                location.reload();
